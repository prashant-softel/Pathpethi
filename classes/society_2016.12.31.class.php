...
class society
{
	...
	public function startProcess()
	{
		...
		if($_REQUEST['insert']=='Submit' && $errorExists==0)
		{
			...
			{				
				...
				{					
					...
					if($res[0]['cnt']==0)
					{
						$insert_society_root = "INSERT INTO `society`(`society_code`, `society_name`, `dbname`, `send_reminder_sms`) VALUES ('" . $_POST['society_code'] . "','" . $this->m_dbConn->escapeString(trim(ucwords($_POST['society_name']))) . "','" . $_SESSION['dbname'] . "', '".$_POST['send_reminder']."')";
						...
						$insert_query = "insert into society (`society_id`, `society_code`, `society_name`, `circle`, `registration_date`, `registration_no`, `society_add`, `city`, `landmark`,
									 ...
									 `calc_int`, `show_share`, `bill_footer`, `sms_start_text`, `sms_end_text`, `send_reminder_sms`,`bill_due_date`,`society_creation_yearid`,`unit_presentation`) values ('" . $result_society_id . "', '".$_POST['society_code']."','".$this->m_dbConn->escapeString(trim(ucwords($_POST['society_name'])))."',
									 ...
									 '".$this->m_dbConn->escapeString(trim(ucwords(     $_POST['society_add'])))."', '".$this->m_dbConn->escapeString(trim(ucwords($_POST['city'])))."',
									 ...
									 '".$_POST['bill_footer']."', '".$_POST['sms_start_text']."', '".$_POST['sms_end_text']."', '".$_POST['send_reminder']."', '".$_POST['bill_due_date']."','".$YearID."','".$_POST['unit_presentation']."')";
		...
		else if($_REQUEST['insert']=='Update' && $errorExists==0)
		{
			...
			{	
				...
				$up_query="update society set `society_name`='".$this->m_dbConn->escapeString(trim(ucwords($_POST['society_name'])))."',`society_add`='".$this->m_dbConn->escapeString(trim(ucwords($_POST['society_add'])))."',
						`registration_date`='" . getDBFormatDate($_POST['registration_date']) . "',`landmark`='".$this->m_dbConn->escapeString(trim(ucwords($_POST['landmark'])))."',
						`city`='".$this->m_dbConn->escapeString(trim(ucwords($_POST['city'])))."',`region`='".$this->m_dbConn->escapeString(trim(ucwords($_POST['region'])))."',
						...
						`sms_start_text` = '".$_POST['sms_start_text']."', `sms_end_text` = '".$_POST['sms_end_text']."', `send_reminder_sms` = '".$_POST['send_reminder']."', `bill_due_date` = '".$_POST['bill_due_date']."',`unit_presentation` = '".$_POST['unit_presentation']."' where society_code='".$_POST['society_code']."' and society_id ='".$_SESSION['society_id']."'";
	...
	{
		$sql1 = "select s.society_id,s.society_code,s.society_name,s.circle,DATE_FORMAT(registration_date, '%d-%m-%Y'),s.registration_no,s.society_add,s.city,s.landmark,s.region,s.postal_code,s.country,s.phone,s.phone2,s.fax_number, s.pan_no, s.tan_no, s.service_tax_no, s.email,s.url, s.member_since, s.bill_cycle, s.int_rate, s.int_tri_amt, s.int_method, s.rebate_method, s.rebate, s.chq_bounce_charge, s.bill_method, s.show_wing, s.show_parking, s.show_area,s.calc_int,s.property_tax_no,s.water_tax_no,s.show_share,s.bill_footer, s.sms_start_text, s.sms_end_text, s.send_reminder_sms, s.bill_due_date,s.unit_presentation, s.cc_email from society as s where s.society_id='".$_REQUEST['societyId']."'";
