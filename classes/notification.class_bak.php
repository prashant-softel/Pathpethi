...
class notification //extends dbop
{
	...
	public function pgnation_bill($society_id, $wing_id, $unit_id, $period_id)
	{
		...
		//return $res;
		$this->display1($result, $period_id);
	}
	
	public function display1($rsas, $periodID)
	{
		...
		$res=$this->show_unit($rsas, $periodID);
		//echo "exiting display1";
		return $res;
	}
	
	public function show_unit($res, $periodID)
	{
		//print_r($res);
		...
		?>
			...
			<td align="center"><iframe src="<?php echo 'pdfbill.php?pdffile=maintenance_bills/' . $res[$k]['society_code'] . '/' . $sqlResult[0]['type']. ' ' . $sqlResult[0]['YearDescription'] . '/bill-' . $res[$k]['society_code'] . '-' . $UnitNo  . '-' . $sqlResult[0]['type']. ' ' . $sqlResult[0]['YearDescription'] . '.pdf'; ?>" name="pdfexport_<?php echo $res[$k]['unit_id']; ?>" id="pdfexport_<?php //echo $res[$k]['unit_id']; ?>" style="border:0px solid #0F0;width:40px;height:40px;"></iframe></td>
            ...
            <input type="button" id="send_email" value="Send Email" onclick="sendEmail(<?php echo $res[$k]['unit_id']; ?>);" />
           	...
           	<td align="center">
			<input type="button" id="send_sms" value="Send SMS" onclick="sendSMS(<?php echo $res[$k]['unit_id']; ?>);" />
