							...
							if($linecount == 1)
							{
							   ...
							   $search_society="select society_id from `society` where society_code='".$society_code."'";
								...
								{ 
									...
									$billDetails_openingBalance_final = $CurrentBillAmount + $SuppbillDetails_openingBalance;
									...
									if($openingBalance)
									{
										if(((float)$billDetails_openingBalance_final) < 0)
										{
											$sqlUpdate = "Update ledger SET opening_balance = '" . abs($billDetails_openingBalance_final) .  "', `opening_type` = '1'  Where id = '" .$unit_id . "'" ;
											$result = $this->m_dbConn->update($sqlUpdate);
						
											$sqlUpdate = "Update assetregister SET Credit = '" . abs($billDetails_openingBalance_final) . "', Debit = 0 where LedgerID = '" .$unit_id . "' and Is_Opening_Balance = 1";
											$result = $this->m_dbConn->update($sqlUpdate);
										...
										{
											$sqlUpdate = "Update ledger SET opening_balance = '" . abs($billDetails_openingBalance_final) .  "', `opening_type` = '2'  Where id = '" .$unit_id . "'" ;
											$result = $this->m_dbConn->update($sqlUpdate);
						
											$sqlUpdate = "Update assetregister SET Debit = '" . abs($billDetails_openingBalance_final) . "', Credit = 0 where LedgerID = '" . $unit_id . "' and Is_Opening_Balance = 1";
											$result = $this->m_dbConn->update($sqlUpdate);
									  ...
									  if(((float)$billDetails_openingBalance_final) < 0)
										{
											...
											$insertAsset = $obj_register->SetAssetRegister(getDBFormatDate($Date), $unit_id, 0, 0, TRANSACTION_CREDIT, abs($billDetails_openingBalance_final), 1);
										}
										else
										{
											$sqlUpdate = "Update ledger SET opening_balance = '" . abs($billDetails_openingBalance_final) .  "', `opening_type` = '2'  Where id = '" .$unit_id . "'" ;
											...
											//echo getDBFormatDate($Date);
											$insertAsset = $obj_register->SetAssetRegister(getDBFormatDate($Date), $unit_id, 0, 0, TRANSACTION_DEBIT, abs($billDetails_openingBalance_final), 1);	
									...
									if($bImportSuppBills)
{
								...
								{ 
									...
									$updatebilldetails .= "where  `UnitID` = '".$unit_id."' and `PeriodID` = '".$result[0]['ID']."' AND BillType = 0";
								  	$this->m_dbConn->update($updatebilldetails);	
									
									$updatebilldetails = "Update `billdetails` set ";
									$updatebilldetails .= "BillSubTotal = '".$SuppBillSubTotal."', ";
									$updatebilldetails .= "BillInterest = '".$SuppBillInterest."', ";
									$updatebilldetails .= "CurrentBillAmount = '".$SuppCurrentBillAmount."', ";
									$updatebilldetails .= "PrincipalArrears = '".$SuppPrincipalArrears2."', ";
									$updatebilldetails .= "InterestArrears = '".$SuppInterestArrears2."', ";
									$updatebilldetails .= "LatestChangeID = '".$changeLogDetail."', ";
									$updatebilldetails .= "TotalBillPayable = '".$SuppTotalBillPayable."' ";
									$updatebilldetails .= "where  `UnitID` = '".$unit_id."' and `PeriodID` = '".$result[0]['ID']."' AND BillType = 1";
								  	$this->m_dbConn->update($updatebilldetails);
								  //'$SuppBillNumber','$SuppBillSubTotal','$SuppBillInterest','$SuppCurrentBillAmount','$SuppPrincipalArrears2','$SuppInterestArrears2','$SuppTotalBillPayable',1
								  	$isImportSuccess = true;
								}
								$errormsg="bill for unit: &lt;".$UnitID." &gt;:: period: &lt;".$prevPeriod." &gt;imported successfully.";
								$this->obj_utility->logGenerator($errorfile,$rowCount,$errormsg,"I");  	
