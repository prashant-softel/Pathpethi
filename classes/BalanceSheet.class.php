				...
				if($amount <> 0)
				{
					...
					else
					{
						...
						if($key['LedgerID'] > 0)
						{
							...
							$tree .= '<a href = "'.$Url.'">'. $key['name']. '</a></span><span style="border:none;width:100px;text-align:right;" class="icon-minus-sign"><a href = "'.$Url.'">'.number_format(abs($amount),2). $amountType.'</a></span></label>';
						}
						else 
						{
...
/*	public function generateIncomeStatement($GroupID,$datas, $parent = 0, $depth = 0, $padding = 10)
	...
	public function OpeningBalanceCalculate()
	{
		...
		{
			$LiabilitySql .= "  and Date = '".getDBFormatDate($_SESSION['default_year_start_date'])."' ";					
		}
		...
		if($_SESSION['default_year_start_date'] <> 0)
		{
			$AssetSql .= "  and Date = '".getDBFormatDate($_SESSION['default_year_start_date'])."' ";					
		}
		...
		if($_SESSION['default_year_start_date'] <> 0)
		{
			$BankSql .= "  and Date = '".getDBFormatDate($_SESSION['default_year_start_date'])."' ";					
		}
