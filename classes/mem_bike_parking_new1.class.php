...
class mem_bike_parking_new1 extends dbop
{
	public $actionPage = "../mem_bike_parking_new1.php?bd&idd=1678";
	public $m_dbConn;
...
$this->display_pg=new display_table($this->m_dbConn);
	...
	public function startProcess()
	{
		...
		{
			if($_POST['bike_reg_no']<>"" && $_POST['bike_owner']<>"" && $_POST['bike_make']<>"" && $_POST['bike_color']<>"")
			{
				$sql = "select count(*)as cnt from mem_bike_parking where member_id='".$_SESSION['member_id']."' and bike_reg_no='".addslashes(trim(strtoupper($_POST['bike_reg_no'])))."' and status='Y'";
				$res = $this->m_dbConn->select($sql);
				
				if($res[0]['cnt']==0)
				{
					$insert_query="insert into mem_bike_parking (`member_id`,`parking_slot`,`bike_reg_no`,`bike_owner`,`bike_model`,`bike_make`,`bike_color`) values ('".$_SESSION['member_id']."','".addslashes(trim(ucwords($_POST['parking_slot'])))."','".addslashes(trim(strtoupper($_POST['bike_reg_no'])))."','".addslashes(trim(ucwords($_POST['bike_owner'])))."','".addslashes(trim(ucwords($_POST['bike_model'])))."','".addslashes(trim(ucwords($_POST['bike_make'])))."','".addslashes(trim(ucwords($_POST['bike_color'])))."')";
