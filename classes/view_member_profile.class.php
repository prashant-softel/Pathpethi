...
$this->display_pg=new display_table($this->m_dbConn);
	...
	public function show_account_details() {
		...
		$sql = "SELECT Sum(Debit) - Sum(Credit) as Dues, ltbl.ledger_name,ltbl.id as ledgerID, loantbl.id,ac.group_id, 'loan' as account_type 
				FROM `assetregister` as assettbl 
				JOIN ledger as ltbl ON assettbl.LedgerID = ltbl.id 
				JOIN pp_loan as loantbl ON loantbl.ledger_id = ltbl.id 
				Join account_category as ac ON ac.category_id = ltbl.categoryid 
				WHERE ltbl.`parent_id` = '".$_GET['id']."' group by assettbl.LedgerID";
		
		$loanResult = $this->m_dbConn->select($sql);

		$sql1 = "SELECT Sum(Debit) - Sum(Credit) as Dues, ltbl.ledger_name, ltbl.id as ledgerID, deposittbl.id,ac.group_id, 'deposit' as account_type 
				FROM `liabilityregister` as liabilitytbl 
				JOIN ledger as ltbl ON liabilitytbl.LedgerID = ltbl.id 
				JOIN pp_deposits as deposittbl ON deposittbl.ledger_id = ltbl.id 
				Join account_category as ac ON ac.category_id = ltbl.categoryid 
				WHERE ltbl.`parent_id` = '".$_GET['id']."' group by liabilitytbl.LedgerID";

		$depositResult = $this->m_dbConn->select($sql1);

		if(empty($loanResult) && !empty($depositResult)){
			return $depositResult;
		}
		else if(!empty($loanResult) && empty($depositResult)){
			return $loanResult;
		}
		else{

			$finalResult = array_merge($loanResult,$depositResult);
			function sortByOrder($a, $b) {
				return $a['ledgerID'] - $b['ledgerID'];
			}
			usort($finalResult, 'sortByOrder');
			return $finalResult;
		}
	}
