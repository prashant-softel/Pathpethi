...
class mem_car_parking_new1 extends dbop
{
	public $actionPage = "../mem_car_parking_new1.php?ccd&idd=1678";
	public $m_dbConn;
...
$this->display_pg=new display_table($this->m_dbConn);
	...
	public function startProcess()
	{
		...
		{
			if($_POST['car_reg_no']<>"" && $_POST['car_owner']<>"" && $_POST['car_make']<>"" && $_POST['car_color']<>"")
			{
				$sql = "select count(*)as cnt from mem_car_parking where member_id='".$_SESSION['member_id']."' and car_reg_no='".$_POST['car_reg_no']."' and status='Y'";
				$res = $this->m_dbConn->select($sql);
				
				if($res[0]['cnt']==0)
				{
					$insert_query="insert into mem_car_parking (`member_id`,`parking_slot`,`car_reg_no`,`car_owner`,`car_model`,`car_make`,`car_color`) values ('".$_SESSION['member_id']."','".addslashes(trim(ucwords($_POST['parking_slot'])))."','".addslashes(trim(strtoupper($_POST['car_reg_no'])))."','".addslashes(trim(ucwords($_POST['car_owner'])))."','".addslashes(trim(ucwords($_POST['car_model'])))."','".addslashes(trim(ucwords($_POST['car_make'])))."','".addslashes(trim(ucwords($_POST['car_color'])))."')";
