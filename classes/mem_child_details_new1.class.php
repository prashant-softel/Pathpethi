...
class mem_child_details_new1 extends dbop
{
	public $actionPage = "../mem_child_details_new1.php?cd&idd=1678";
	public $m_dbConn;
...
$this->display_pg=new display_table($this->m_dbConn);
	...
	public function startProcess()
	{
		...
		{
			if($_POST['child_name']<>"" && $_POST['child_desg']<>"" && $_POST['child_bg']<>"")
			{
				$sql = "select count(*)as cnt from mem_child_details where member_id='".$_SESSION['member_id']."' and status='Y'";
				$res = $this->m_dbConn->select($sql);
				
				$insert_query = "insert into mem_child_details (`member_id`,`child_name`,`child_desg`,`child_dob`,`scc`,`sdd`,`child_bg`) values ('".$_SESSION['member_id']."','".addslashes(trim(ucwords($_POST['child_name'])))."','".$_POST['child_desg']."','".$_POST['child_dob']."','".addslashes(trim(ucwords($_POST['scc'])))."','".addslashes(trim(ucwords($_POST['sdd'])))."','".addslashes(trim(ucwords($_POST['child_bg'])))."')";
