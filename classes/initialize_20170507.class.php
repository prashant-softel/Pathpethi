...
class initialize
{
		...
		try
	  {
		  /* $transport = Swift_SmtpTransport::newInstance('cs10.webhostbox.net', 465, "ssl")
			...
			if($bIsSendSocistyDetails == true && $data <> "" && count($data) > 0)
			{
				...
				$mailBody .= "<table  align='center' border='1'  style='border-collapse: collapse;'><tr><th style='padding: 5px 5px 5px 5px;'>Society Name</th><th style='padding: 5px 5px 5px 5px;'>Wing</th><th style='padding: 5px 5px 5px 5px;'>Unit/Flat/Shop No</th></tr>
        							<tr><td  style='padding: 5px 5px 5px 5px;'>".$data['society_name']."</td><td  style='padding: 5px 5px 5px 5px;'>".$data['wing']."</td><td  style='padding: 5px 5px 5px 5px;'>".$data['unit']."</td></tr></table>";	
	...
	public function sendNewUserActivationEmail($email ,  $name ,$sAccountActivationCode ="", $GetReturnStatus ="")
	{
		//echo "sending email";
		include_once('email_format.class.php');
		require_once('../swift/swift_required.php');
