"use strict";
const CASH_MODE   = $('#cash_account').val();
const CHEQUE_MODE = $('#cheque_account').val();;
...
function getsociety(str)
{
	...
	if(iden[0]=="delete")
	{
		...
		if(d==1)
		{
			remoteCall("ajax/ajaxdeposits.php","form=society&method="+iden[0]+"&societyId="+iden[1],"loadchanges");
		}
	}
...
function loadchanges()
{
	...
	else if(arr1[0] == "show")	
	{
		...
		for (var i = 0; i < arr2.length - 1; i++){
   			option += '<option value="'+ arr2[i] + '">' + arr2[i] + '</option>';
		}
		$('#introducer_1').append('<option value="please_select">PLEASE SELECT</option>' + option);
		$('#introducer_2').append('<option value="please_select">PLEASE SELECT</option>' +option);
...
function val()
{
	...
	if(document.getElementById("insert").value == "Edit")
	{
		return true;	
	}
	//////////////////////////////////////////////////////////////////////////////////////////
...
$(document).on('focusout', '.calculate_field', function(){

	let interest_rate   = $('#int_rate').val();
	let installment_amt = $('#installment_amt').val();
	let deposit_date = $("#deposit_date").val();
	let maturity_date = $("#deposit_maturity_date").val();


	

	if(installment_amt && interest_rate && deposit_date && maturity_date){

		
		let starting_date = getDateObject(deposit_date);
		let ending_date = getDateObject(maturity_date);
		console.log(starting_date, ending_date);
		installment_count = getMonthDiff(starting_date, ending_date);
		let deposit_amt = installment_amt * installment_count;
		let maturity_amt = Math.pow(deposit_amt * ((1 + (interest_rate/100)/12)), installment_count);
		console.log(maturity_amt);
		console.log(interest_rate, installment_amt, deposit_date, maturity_date);
...
$(document).on('change', '#receipt_type', function(){
	...
	if(receipt_type == CHEQUE_MODE){
		$('.chequeDiv, .bankLedger').removeClass('hidden');
		$('.cashLedger').addClass('hidden');
	}
	else{
		$('.chequeDiv, .bankLedger').addClass('hidden');
		$('.cashLedger').removeClass('hidden');
	}
});